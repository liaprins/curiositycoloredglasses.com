...
function lightbox(imgToShow) {
    ...
    singleLightbox.innerHTML = singleLightbox.parentNode.innerHTML + '<img src="https://curiositycoloredglasses.com/assets/images/sitewide/x.svg" alt="close" id="lightboxclose" class="close-x yellowhover" data-lightbox-x>';
    ...
    if (lightboxImg.nextElementSibling.classList.contains('imgtofullscreen')) {
        ...
        singleImgToRegView.setAttribute('src', '/assets/images/sitewide/to-regular-view.svg');
    ...
    if (caption) {
        ...
        captionToggle.setAttribute('src', '/assets/images/sitewide/up-arrowhead.svg');
...
function populateLightboxDots(imgToShow) { // imgToShow is the image that should be shown within the lightbox
    ...
    if (imgToShow.getAttribute('id') == ('galleryimage')) {
        ...
        lightboxArrowContainer.innerHTML = '<img src="https://curiositycoloredglasses.com/assets/images/sitewide/left-arrowhead.svg" alt="retreat" title="Previous image" class="galleryarrows lightboxarrows yellowhover" data-retreatlightboxarrow>' + '<img src="https://curiositycoloredglasses.com/assets/images/sitewide/right-arrowhead.svg" alt="advance" title="Next image" class="galleryarrows lightboxarrows yellowhover" data-advancelightboxarrow>' + '<img class="imgfullscreentoggle imgtoregview yellowhover" title="View larger" id="galleryimgtoregview" alt="View larger" src="https://curiositycoloredglasses.com/assets/images/sitewide/to-regular-view.svg">';
...
function advanceOrRetreat(clickedSideSlide, dotsContainer, gallery, clickedIndex, galleryName, currentSlide) {
    ...
    for (k = 0; k < dotsList.length; k++) {
        if (k == clickedIndex) {
            ...
            dotsList[k].innerHTML = '<img src="https://curiositycoloredglasses.com/assets/images/sitewide/dot-filled.svg" alt="go to this slide" class="dotimg dotimgfill">';
        } else {
            dotsList[k].setAttribute('class', 'dot dotunfilled');
            dotsList[k].innerHTML = '<img src="https://curiositycoloredglasses.com/assets/images/sitewide/dot.svg" alt="go to this slide" class="dotimg dotimgunfilled">';
        } // close if
    } // close k
...
function verticallyCenter() {
    ...
    if (singleLightbox) {
        ...
        var screenWidth = window.innerWidth; // get screen width
        if (screenWidth >= 1225) {
          if ((imgToShow.offsetWidth / imgToShow.offsetHeight) > (1108 / (singleLightbox.offsetHeight - 94))) {
            var lightboxImgHeight = (imgToShow.offsetHeight * 1108) / imgToShow.offsetWidth;
            lightboxImg.style.marginTop = (lightboxHeight - lightboxImgHeight)/2 + 'px';
          } else {
            lightboxImg.style.marginTop = '2rem';
          }
        } else if (screenWidth >= 817) {
          if ((imgToShow.offsetWidth / imgToShow.offsetHeight) > ((singleLightbox.offsetWidth - 117) / (singleLightbox.offsetHeight - 94))) {
            var lightboxImgHeight = (imgToShow.offsetHeight * (singleLightbox.offsetWidth - 117)) / imgToShow.offsetWidth;
            lightboxImg.style.marginTop = (lightboxHeight - lightboxImgHeight)/2 + 'px';
          } else {
            lightboxImg.style.marginTop = '2rem';
          }
