    ...
    if (clickedImg.classList.contains('clickme')) {
        ...
        if (caption) {
            ...
            if (clickedImg.getAttribute('id') == ('galleryimage')) {
                ...
                /*
                // pull in surrounding imgs (maybe only for 1225+ screenwidth)
                currentIndex = clickedImg.parentNode.parentNode.getAttribute('data-slide-index');
                galleryLength = clickedImg.parentNode.parentNode.parentNode.children.length - 2;    // minus 2 to get rid of dotsContainer and placeholderBox
                RSideSlideCount = galleryLength - currentIndex - 1;    // minus 1 to account for indexes starting at 0
                LSideSlideCount = currentIndex;    // would subtract 1, but no need, because index starts at 0, so offsets everything to be 1 less already
                RFirstSlide = clickedImg.parentNode.parentNode.nextElementSibling.firstElementChild;
                */
...
// TESTING GALLERY LIGHTBOX vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
/*
...
// window.addEventListener('click', openGalleryLightbox, false);
