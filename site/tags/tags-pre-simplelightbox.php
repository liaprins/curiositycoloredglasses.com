...
kirbytext::$tags['image'] = array(
  ...
  'html' => function($tag) {
    ...
    $file    = $tag->file($url);
    /* $metacaption = $file->caption(); */    // <<< ADDED BY LIA! This collects caption from the image file's metadata file, but was causing breaking errors if the panel called a non-existent image. Leaving it for now in case of emergency, but commented out!
    ...
    $_image = function($class) use($tag, $url, $alt, $title) {
      return html::img($url, array(
        ...
        'class'  => $tag->attr('class', 'contentimage singleimage'), //ADDED BY LIA If there is not a class supplied inline in the panel (via "class: xxx") then this class I already made, called "contentimage" will be applied, otherwise it will be the supplied class
